...
void Search::DisplayResults() const {
    ...
    // Создаем форматированную таблицу результатов
    ...
    cout << "\n" << left
        << setw(5) << "№"
        << setw(nameWidth) << "Название"
        << setw(brandWidth) << "Бренд"
        << setw(priceWidth) << "Цена"
        << setw(qtyWidth) << "Кол-во"
        << "Статус" << endl;

    cout << string(5 + nameWidth + brandWidth + priceWidth + qtyWidth + 10, '-') << endl;

    for (int i = 0; i < resultCount; i++) {
        // Обрезаем длинные названия
        string displayName = searchResults[i].name;
        if (displayName.length() > nameWidth - 3) {
            displayName = displayName.substr(0, nameWidth - 3) + "...";
        }

        string displayBrand = searchResults[i].brand;
        if (displayBrand.length() > brandWidth - 3) {
            displayBrand = displayBrand.substr(0, brandWidth - 3) + "...";
        }

        cout << left
            << setw(5) << (i + 1)
            << setw(nameWidth) << displayName
            << setw(brandWidth) << displayBrand
            << setw(priceWidth) << (to_string(searchResults[i].price) + " руб.")
            << setw(qtyWidth) << searchResults[i].quantityStock
            << (searchResults[i].statusStock ? "В наличии" : "Нет в наличии") << endl;

        // Дополнительная информация для первого товара
        if (i == 0 && !searchResults[i].description.empty()) {
            cout << "   Описание: ";
            if (searchResults[i].description.length() > 50) {
                cout << searchResults[i].description.substr(0, 50) << "..." << endl;
            }
            else {
                cout << searchResults[i].description << endl;
            }
        }
    }
